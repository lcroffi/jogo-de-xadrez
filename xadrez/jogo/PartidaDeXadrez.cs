...
namespace xadrez.jogo {
    class PartidaDeXadrez {
        ...
        public void realizaJogada(Posicao origem, Posicao destino) {
            ...
            if (testeXeque(adversaria(jogadorAtual))) terminada = true;
            else {
                turno++;
                mudaJogador();
            }
        ...
        public void validarPosicaodeOrigem(Posicao pos) {
            if (tab.peca(pos) == null) {
                throw new TabuleiroException("Não há nenhuma peça na posição de origem escolhida.");
            }
            if (jogadorAtual != tab.peca(pos).cor) {
                throw new TabuleiroException("Não é o turno desse jogador.");
            }
                ...
                for (int i = 0; i<tab.linhas; i++) {
                    for (int j = 0; j<tab.colunas; j++) {
                        if (mat[i, j]) {
                            ...
                            Peca pecaCapturada = executaMovimento(origem, destino);
                            bool testandoXeque = estaEmXeque(cor);
                            desfazMovimento(origem, destino, pecaCapturada);
                            if (!testandoXeque) return false;
                        }
                    }
                }
            }
